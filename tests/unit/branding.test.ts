...
describe("white-label: normalización", () => {
  it("nombre vacío o nulo → default 'Reservas CRM'; se recorta a 30", () => {
    expect(normalizeBranding(null).name).toBe("Reservas CRM");
    expect(normalizeBranding({ name: "   " }).name).toBe("Reservas CRM");
