...
export function InstagramConnectionCard() {
  ...
  return <div className="max-w-3xl space-y-6"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Instagram className="h-5 w-5" /> Instagram</CardTitle><CardDescription>Conecta una cuenta profesional desde el instalador de reservas.AI. Los tokens nunca se muestran en el CRM.</CardDescription></CardHeader><CardContent className="space-y-4">{state?.connected && state.account ? <div className={`flex items-center gap-3 rounded-lg border p-4 ${healthy ? "border-[#d8e8dd] bg-[#eff7f1]" : "border-[#ece2cf] bg-[#faf7f0]"}`}>{healthy ? <CheckCircle2 className="h-5 w-5 text-success" /> : <AlertTriangle className="h-5 w-5 text-[#8a6d3b]" />}<div className="flex-1 text-sm"><p className="font-medium">{state.account.displayName ?? state.account.username ?? "Cuenta de Instagram"}</p><p className="text-muted-foreground">{state.account.username ? `@${state.account.username}` : state.account.id}</p></div><Badge variant={healthy ? "success" : "warning"}>{healthy ? "Conectado" : "Revisar"}</Badge></div> : <div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">No hay una cuenta conectada. Usa el instalador de la plataforma para iniciar OAuth con Meta.</div>}<div className="grid gap-3 rounded-md border bg-background/40 p-4 text-sm"><p className="font-medium">Estado operativo</p><p className="flex items-center gap-2 text-muted-foreground"><ShieldCheck className="h-4 w-4" /> Webhook: {state?.account?.webhookStatus ?? "pendiente"}</p><p className="text-xs text-muted-foreground">Permisos iniciales: basic, gestionar comentarios y gestionar mensajes. La aprobación de Meta sigue siendo un requisito para cuentas externas.</p></div></CardContent></Card></div>;
}
