...
- Reservas CRM es genérico: cada instancia lo opera un negocio distinto → la
  marca visible (nombre + acento) debe ser suya, no la del proyecto base.
...
4. Filtros: "Todas" y "No leídas" con conteos (Reservas CRM no tiene asignación de
...
**Aceptación**:
1. Nueva pestaña Configuración → Marca: input de nombre (default "Reservas CRM") y
...
4. Sin configurar → defaults (nombre "Reservas CRM", acento azul acero `#3f5972`).
...
Reservas CRM), logo por imagen (v1: inicial sobre acento), densidad configurable.
